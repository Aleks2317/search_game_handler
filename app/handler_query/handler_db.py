...
async def search_get_db(query: str) -> SearchResult:
    ...
    try:
        async for db in get_db():
            ...
            some_query_games = select(Game.id, Game.provider_id).where(
            ...
            if rows_games:
                ...
                result.games, result.providers = list(games_id), list(
                    providers_id
                )

            some_query_providers = select(Provider.id).where(
            ...
            if rows_providers:
                result.providers = [
                    ...
                    if row[0] is not result.provider_id
                ]
